...
const Header = () => {
    ...
    return (
        <div className={` w-full fixed   md:px-10 px-7 top-0 left-0 z-50 ${MouseMoveY == 0 ? ' py-8 bg-black  ' : ' shadow-md py-5 color-beige'} ease-out duration-500 `}  >
            <div className=' flex md:flex items-center justify-between'>
                ...
                <ul className={` ${MouseMoveY == 0 ? ' ' : 'color-beige text-zinc-50 '} md:gap-[1.5rem] text-[16px] text-zinc-300 bg-black md:flex md:items-center font-poppins font-thin  md:pb-0 pb-12 absolute md:static md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9  ease-out duration-500 ${open ? 'top-12' : 'top-[-490px]'}`}>
                    {
                        urls.map((link, i) => (
                            <li key={i} className=' md:my-0 my-7 '>
